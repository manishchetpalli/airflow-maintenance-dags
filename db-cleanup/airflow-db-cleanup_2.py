...
def analyze_db():
    session = settings.Session()
    try:
        logging.info("Starting to analyze tables individually.")
        for db_object in DATABASE_OBJECTS:
            table_name = db_object["airflow_db_model"].__tablename__
            analyze_stmt = f"ANALYZE TABLE {table_name}"
            logging.info(f"Analyzing table: {table_name}")
            session.execute(text(analyze_stmt))
        session.commit()
        logging.info("Finished analyzing tables.")
    except Exception as e:
        logging.error("Error during table analysis.")
        logging.exception(e)
    finally:
        session.close()
